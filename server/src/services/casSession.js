/**
 * CAS 会话服务：管理强智教务网的 CAS 统一认证与模拟登录状态，实现多请求共享会话与自动重新登录。
 */
...
// NOTE: 缓存 Session，保存在 Node.js 内存中
...
async function performLogin() {
  ...
  try {
    const username = config.FOSU_SERVICE_USERNAME;
    const password = config.FOSU_SERVICE_PASSWORD;

    if (!username || !password) {
      throw new Error("NEED_LOGIN: 请在后端配置教务网登录账号 FOSU_SERVICE_USERNAME 与 FOSU_SERVICE_PASSWORD");
    }
    ...
    const serviceUrl = `${config.FOSU_BASE_URL}/caslogin.jsp?kstzType=null`;
    const loginUrl = `${config.FOSU_AUTH_URL}/authserver/login?service=${encodeURIComponent(serviceUrl)}`;
