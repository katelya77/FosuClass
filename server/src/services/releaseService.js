...
function buildReleasePackFilesMeta(files) {
  ...
  [
    files.bootstrapPath,
    path.join(files.releaseDir, "calendar.json"),
  ].forEach((filePath) => {
...
function writeReleaseSnapshot(rawSnapshot) {
  ...
  const calendar = snapshot.teachingCalendar || teachingCalendarService.readTermCalendar(snapshot.term || snapshot.semester);
  teachingCalendarService.writeReleaseCalendar({
    term: snapshot.term || snapshot.semester,
    releaseVersion: version,
  }, {
    calendar,
...
function getActiveReleaseInfoFast() {
  ...
  return Object.assign({}, active, {
    ...
    term: pointer?.activeTerm || pointer?.term || summary?.term || semester,
    semester: pointer?.activeTerm || pointer?.term || summary?.semester || semester,
...
function getReleasePackQuickHealth(version) {
  ...
  const keyFiles = {
    ...
    calendar: path.join(files.releaseDir, "calendar.json"),
    staticCalendar: path.join(files.publicReleaseDir, "calendar.json"),
  };
  ...
  const requiredOk =
    ...
    (checks.calendar.exists || checks.staticCalendar.exists);
...
async function rebuildReleasePackAsync(version, options = {}) {
  ...
  try {
    ...
    const calendar = normalizedSnapshot.teachingCalendar || teachingCalendarService.readTermCalendar(normalizedSnapshot.term || normalizedSnapshot.semester);
    teachingCalendarService.writeReleaseCalendar({
      term: normalizedSnapshot.term || normalizedSnapshot.semester,
      releaseVersion: normalizedVersion,
    }, {
      calendar,
      releaseDir: files.releaseDir,
