  ...
  try {
    ...
    if (active !== "public" && envSnapshot.agentEnabled) {
      ...
      probe = {
        ...
        // never write probe into user conversations/memory
    ...
    if (envSnapshot.reasonCode === "AGENT_DISABLED") suggestions.push("打开 AI_AGENT_ENABLED=true（仅 trial/dev Profile）。");
    if (envSnapshot.reasonCode === "PROVIDER_MOCK") suggestions.push("将 AI_PROVIDER 设为 deepseek/coze/cloudbase-openai，并配置 Key。");
    if (envSnapshot.reasonCode === "PROVIDER_KEY_MISSING") suggestions.push("补全 Provider API Key，并确认加密配置可读。");
    if (envSnapshot.reasonCode === "PROVIDER_EXPIRED") suggestions.push("Coze 临时 Provider 已到期，请续期或从 Provider Chain 移除。");
    if (readiness.reasonCode === "ENHANCED_SESSION_NOT_AUTHORIZED") suggestions.push("配置 AI_COMPETITION_OPENID_HASH_PREFIXES 或关闭 AI_ENHANCED_REQUIRE_SESSION（仅开发）。");
    if (readiness.reasonCode === "TRIAL_ENV_NOT_ALLOWED") suggestions.push("确认小程序 envVersion 为 develop/trial，并设置 AI_COMPETITION_ALLOW_TRIAL_ENV=true。");
