...
function verifySignedRequest(req, now) {
  ...
  const agentId = headerValue(req.headers, "x-campus-agent-id");
  if (!agentId || agentId !== secrets.agentId) return { ok: false, status: 404 };
  const timestamp = headerValue(req.headers, "x-campus-timestamp");
  const nonce = headerValue(req.headers, "x-campus-nonce");
  const signature = headerValue(req.headers, "x-campus-signature");
