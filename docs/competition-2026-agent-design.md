# 2026 校园工具应用方案：佛课小表校园服务管家

## 赛道

2-E 数智生活：综合服务应用开发 / 校园服务。

## 作品定位

《佛课小表·校园服务管家》面向佛山大学学生的课程与空间服务场景。它不把外部内容当成教务事实来源，而是在现有课表、Release Pack、全校索引、空教室索引和个人 XLS 课表摘要之上，新增自然语言任务层，让学生用一句话完成“查课、找教室、看今日安排、推荐自习时间、排查数据问题”等高频校园任务。
...
```mermaid
flowchart TD
  A["微信小程序校园助手页"] --> B["/api/ai/agent/chat"]
...
- 校园服务管家页面：快捷问题、查询输入、结构化卡片、后续建议、本地最近 20 条查询记录。
...
- 个人导入指引：优先推荐 XLS，不要求用户把学号密码输入查询框。
...
- Provider 角色：只做表达与摘要，不作为课程、教师、教室或空教室事实来源。

## 创新点

- 工具优先的校园服务链路：事实由确定性工具产生，Provider 只负责组织表达。
- 无 key 可演示：默认 mock provider，现场断网或未配置 Provider 仍能跑通核心场景。
- 数据最小化：个人课表只上传课程名、教师、教室、星期、节次、教学周等必要字段。
- 卡片化任务面板：不是纯查询框，而是能直接跳转到小程序业务页的行动界面。
- 比赛合规优先：mock/local 可演示、国产 Provider 脱敏调用、未来境内部署生产模式可迁移，敏感信息脱敏且日志不记录原文。

## 参赛部署叙事

本项目不把 Oracle ARM 或海外 VPS 表述为境内合规部署。参赛材料按三种模式描述：

- 本地/评审机 mock 演示模式：默认可用，不调用外部 Provider。
- 国产 Provider 脱敏调用模式：DeepSeek/Coze 只接收工具结果和最小上下文，DeepSeek 默认 `deepseek-v4-flash`，复杂说明可切 `deepseek-v4-pro`。
...
| 评分维度 | 对应设计 |
| --- | --- |
| 应用创新 | 课程、空间、个人安排和数据诊断融合为校园服务工具 |
| 技术完整度 | 小程序页面、Express 路由、工具注册、Provider fallback、测试脚本完整 |
| 场景价值 | 覆盖查课、找空教室、今日安排、组会自习、导入指引等真实需求 |
| 安全合规 | 三模式边界说明、数据最小化、敏感信息脱敏、不保存密码 |
| 可演示性 | mock provider 默认可用，无 Provider key 也能返回卡片 |
