...
- 混元只用于项目说明、使用帮助、自然表达和对既有工具结果的非事实性解释。
...
CloudBase 官方小程序接入文档要求微信基础库不低于 `3.15.1`。运行时会检查：
...
- Provider 返回的 actions 一律丢弃。
- 说明类结果只允许 `answer`、普通 `generic` card 和 suggestions。

## 并发与限额

体验服务单环境并发上限为 5，因此小程序端执行：
...
- 单设备每日说明类软限制默认 20 次；工具查询不受影响。

## 公开发布合规开关

- `AI_COMPETITION_MODE=true`：开发版、体验版、比赛演示可用混元。
- `AI_GENERATIVE_PUBLIC_ENABLED=false`：正式公开版默认关闭开放式说明查询。
- `AI_TOOL_ONLY_MODE=true`：只保留确定性工具。

如果正式公开版资质尚未确认，页面提示“说明查询暂未开放，校园工具仍可正常使用”，不影响查课、空教室、今日课程和数据诊断。
