...
| `FOSU_AI_CONFIG_ENCRYPTION_KEY` | 无 | Provider 运行时配置加密密钥 | 部署前置检查失败，只输出变量名 |
| `FOSU_AGENT_MEMORY_SECRET` | 无 | 会话 Principal、云端记忆与 Coze 匿名 ID 的独立 HMAC 密钥 | 部署前置检查失败，只输出变量名 |
| `FOSU_AGENT_REMINDER_SECRET` | 无 | 提醒状态 AES-GCM 与确认令牌签名密钥 | 部署前置检查失败，只输出变量名 |
| `FOSU_WECHAT_RECIPIENT_SECRET` | 无 | OpenID 接收者保险箱独立 AES-GCM 密钥 | 部署前置检查失败，只输出变量名 |
...
| `FOSU_COURSE_REMINDER_DISPATCH_INTERVAL_MS` | `60000` | 到期提醒扫描周期 | 最小 30000 ms |
