...
核心 Secrets：`VPS_HOST`、`VPS_USER`、`VPS_SSH_KEY`、`VPS_APP_DIR`、`ADMIN_API_TOKEN`、`ADMIN_PASSWORD`、`ADMIN_TOKEN`、`FOSU_AI_CONFIG_ENCRYPTION_KEY`、`FOSU_AGENT_MEMORY_SECRET`、`FOSU_AGENT_REMINDER_SECRET`、`WECHAT_APPID`、`WECHAT_APPSECRET`、`FOSU_SESSION_SECRET_CURRENT`、`FOSU_CSRF_SECRET`。按启用能力再配置 `OPENROUTER_API_KEY`、`AI_API_KEY`/`DEEPSEEK_API_KEY`、`CLOUDBASE_OPENAI_API_KEY` 或 `COZE_API_KEY`，以及静态票据、导入中继和提醒相关密钥。
