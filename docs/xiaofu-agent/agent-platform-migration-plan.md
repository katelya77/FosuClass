...
| M6 | ✅ 代码与测试完成（待 commit） | `output/agent-platform-m6-progress.md`；意图→工具映射单源（五因子交集 + router 兜底）+ kernel 旧链移除单一 PlannerCoordinator + 恢复逻辑下放 skill + MAX_REPLAN 口径一致（≤2）+ durable 层（哈希 token、跨进程 resume、提醒 receipt_wait 接线）；五门禁 + phase2 全绿（regression 126/126）。遗留：durable 无定期 sweep（惰性过期已保证正确性） |
| M7 | 未开始 | — |
