...
| M4 | ✅ 代码与测试完成（待 commit） | `output/agent-platform-m4-progress.md`；followUpResolver 唯一 V2 实现（11 组测试）+ 三处旧解析器全退役（grep 唯一性证据）+ working state 八字段对齐（pendingClarification 5 键统一形态三方一致）；phase2 11/11、foundation 33/33；regression 123/123（T2 期间）。遗留：T1 锁定 2 处【疑似缺陷】（parseDateOffset 分支顺序、legacy 空教室 inherited/replaced 双列）维持锁定未修 |
| M5–M7 | 未开始 | — |
