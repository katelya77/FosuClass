# 小佛校园助手校园信息库与工具分流方案（人工维护版）

> 用途：把本文件作为 Codex GPT-5.5 xhigh 的上下文资料，让它据此重构/增强小佛校园助手的信息库、卡片系统、前端交互和工具分流。
...
### 0.1 输入框 UI 问题
当前校园服务管家底部输入框 placeholder 出现自动换行，例如：
...
- 推荐 placeholder：
  ...
  - `问课表、校园入口和办事`
  - `问小佛校园事项`
- 输入框区域需要保持高度稳定，避免键盘弹出/查询记录滚动时跳动。

### 0.2 知识库过浅
现在小佛校园助手的校园信息库偏像“临时 FAQ”，而不是一个可维护的校园信息底座。需要：
...
### 0.3 查询与卡片不在同一条线
用户提交的查询，回答、卡片、按钮、复制、跳转都必须在同一条查询流内出现。弹层、任务面板、快捷 chips 只是入口，不是另一个结果频道。
...
> 这些条目用于启动小佛校园助手的信息查询能力。Codex 应继续拆成结构化数据，并标注来源与可信度。
...
- confidence: `high`
- summary: 导入个人课表后，小佛校园助手才能查询“今天有什么课”“明天有什么课”“下一节课在哪里”等个性化问题。
- content:
  - 不要在查询框输入学号和密码。
...
### 6.4 查询在一条线上
- 点击快捷任务后，应在当前查询流生成一条“用户问题”与一条“查询结果”。
- 弹层关闭后自动回到查询流。
- 不要让用户感觉“点任务”和“查询结果”是两个独立系统。
