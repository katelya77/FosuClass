...
校园助手不保存密码，不要求用户在对话里输入或粘贴学校密码，也不读取个人课表同步页的密码字段。用户可以在个人课表同步页面自己输入学号和密码，完成客户端直连同步；这不等于把凭据交给 AI。AI 上下文、工具参数和 Provider 请求都不得包含密码、Cookie、ticket、execution 或 pwdEncryptSalt。旧的服务端登录抓取接口仍返回 `410 XLS_ONLY`。XLS 导入继续可用，XLS 原始文件内容和 base64 不进入 Provider。
