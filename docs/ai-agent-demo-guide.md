# 校园服务管家演示指南

校园服务管家是面向校园生活场景的工具驱动助手。演示时应突出“事实来自确定性工具，结果由本地规则和校园数据整理”。

## 稳定演示场景

1. 推荐连续自习时间和空教室。
2. 用自然语言查询教师、班级、教室或课程。
3. 生成个性化周学习计划。
4. 辅助安排会议、识别冲突并给出决策建议。

演示模式返回会带有明确标记：

```json
{
  "safety": { "demoData": true },
  "evidence": { "releaseVersion": "demo-data", "sources": ["demo-data"] }
}
```

演示数据不得混入生产 Release 文件。

## 工具优先原则

课程、空教室、索引、校历、当前教学周和数据诊断等事实，必须由确定性校园工具提供。DeepSeek、Coze 或其他已配置的国内 Provider 可用于意图理解和语言润色，但 Provider 文本不能覆盖工具事实。

当 Provider 不可用时，确定性工具仍应继续工作；界面只提示智能化表述暂时不可用。

## 个性化边界

个人课表上下文保存在本地，并且必须由用户主动开启。小程序需要提供查看记录、暂停个性化、删除个性化和清空查询的入口。任何 Provider 请求前都要先脱敏身份字段。

## 证据卡片

查询结果包含轻量 `taskSteps` 和 `evidence`：

- 已理解需求。
- 已读取课表。
- 已检查空教室。
- 已查询索引。
- 已生成建议。
- 记录学期、Release 版本、当前教学周、来源模式和检查时间。

结果中不得出现 `[object Object]`、编造教室、编造教师课表、原始 token/session 值，或把旧 Release 数据描述成实时数据。
