...
- 校园服务管家：常用任务与更多任务统一改用本地 SVG 图标；更多任务分组打开时才写入页面 `data`，并缓存到 `FOSU_AI_TASK_PANEL_GROUPS_CACHE`。
- 课程卡片：课程类结果优先展示具体时间段，例如 `08:00-09:25`；节次保留在副标题中。
...
标题：`[codex] 优化小程序设置页、校园管家与课表缓存`

正文要点：

- 精简设置页首屏，合并课表绑定、导入和学期信息。
- 任务入口改为 SVG 图标，更多任务按需渲染并加入防抖。
