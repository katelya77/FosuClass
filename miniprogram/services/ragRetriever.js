...
const CATEGORY_HINTS = {
  ...
  weather: ["天气", "下雨", "带伞", "温度"],
  app_help: ["小佛", "小佛AI", "校园管家", "怎么用", "能做什么"],
...
function isHelpQuery(query) {
  const value = normalizeText(query);
  if (!value) return false;
  return /(怎么用|如何使用|使用帮助|小佛能做什么|能做什么|我能问你什么|如何导入|怎么导入|导入个人课表|xls|数据来源说明)/.test(value);
}
...
function isDynamicToolQuery(query) {
  ...
  return isWeatherToolQuery(value) ||
    ...
    /(今天有什么课|明天有什么课|本周课表|下一节课|查班级|查教师|查老师|查教室|查课程|导入个人课表|小佛能做什么|怎么用|如何使用)/.test(value);
}
