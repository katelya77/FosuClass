...
const MANIFEST_PATH = path.resolve(__dirname, "../../config/admin-rollout-manifest.json");
...
function validateEvidence(value, field, moduleName) {
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item.trim())) fail("modules." + moduleName + "." + field + " must be a string array");
}
function validateManifest(manifest) {
  ...
  for (const [moduleName, moduleConfig] of Object.entries(manifest.modules)) {
    ...
    for (const field of REQUIRED_MODULE_FIELDS) if (!(field in moduleConfig)) fail("modules." + moduleName + "." + field + " is required");
    ...
    modules[moduleName] = Object.freeze({ productionWriteEnabled: moduleConfig.productionWriteEnabled, routes: Object.freeze(normalizedRoutes), httpEvidence: Object.freeze(moduleConfig.httpEvidence.slice()), browserEvidence: Object.freeze(moduleConfig.browserEvidence.slice()) });
  }
  if (!Object.keys(modules).length) fail("modules must not be empty");
  return Object.freeze({ schemaVersion: manifest.schemaVersion, rolloutVersion: manifest.rolloutVersion, admin: Object.freeze({ primary: manifest.admin.primary, nextEnabled: manifest.admin.nextEnabled }), imageTarget: manifest.imageTarget, modules: Object.freeze(modules) });
}
let cachedManifest;
function loadAdminRolloutManifest() {
  if (cachedManifest) return cachedManifest;
  let parsed;
  try { parsed = JSON.parse(fs.readFileSync(MANIFEST_PATH, "utf8")); } catch (error) { fail("unable to read " + MANIFEST_PATH + ": " + error.message); }
...
function resolveWriteModule(routePath) {
  ...
  const matches = [];
  for (const [moduleName, moduleConfig] of Object.entries(loadAdminRolloutManifest().modules)) if (moduleConfig.routes.some((route) => normalized === route || normalized.startsWith(route + "/"))) matches.push(moduleName);
  return matches.length === 1 ? matches[0] : null;
}
function getKnownWriteModules() { return Object.freeze(Object.keys(loadAdminRolloutManifest().modules)); }
...
module.exports = { MANIFEST_PATH, loadAdminRolloutManifest, resolveWriteModule, getKnownWriteModules, getProductionWriteModules };
