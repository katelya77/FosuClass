/**
 * 快路径（低延迟）契约测试：
 * 1) 显式 adaptive 下事实类意图本地高置信命中 → 跳过 Decision 模型调用（deterministic_adaptive），
 ...
 * 4) 个人记忆问句 → 统一 Decision 后由本地 Memory 响应，不调用 Response 模型
...
deepseekProvider.generateStructured = async (input) => {
  ...
  return {
    ...
    content: JSON.stringify({
      ...
      goal: { name: "conversational_help", confidence: 0.9, requiresClarification: false },
      entities: [],
      constraints: {},
      skillCandidates: [{ skillId: "knowledge_search", confidence: 0.9 }],
      plan: { steps: [{ id: "respond", skillId: "knowledge_search", purpose: "Respond conversationally" }] },
...
async function run() {
  ...
  assert.strictEqual(fact.understanding.source, "deterministic_adaptive", "fact Decision skipped model only under explicit adaptive");
  ...
  assert.strictEqual(qa.understanding.source, "deterministic_adaptive", "project_qa Decision skipped model under adaptive");
  ...
  assert.ok(structuredPurposes.includes("decision"), "unmatched chat must keep the unified model Decision");
  ...
  // 4) 个人记忆问句：仍遵守统一 Decision，随后由本地 Memory 响应。
  structuredPurposes.length = 0;
  responseProviderCalls = 0;
  const memory = await chat("我叫什么名字");
  assert.strictEqual(intentNameOf(memory), "conversation_memory");
  assert.deepStrictEqual(structuredPurposes, ["decision"], "personal memory turn must keep the unified Decision contract");
