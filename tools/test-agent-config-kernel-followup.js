...
if (MODE === "data-dir") {
  ...
  const diagnostics = composition.getConfigKernel().diagnostics("public");
...
async function main() {
  ...
  const publicSnapshot = kernel.getCurrentSnapshot("public");
  const catalog = composition.resolveSkillCatalogForSnapshot(publicSnapshot);
  ...
  const trialSnapshot = kernel.getCurrentSnapshot("trial");
  ...
  const recovered = composition.resolveSkillCatalogForSnapshot(trialSnapshot);
