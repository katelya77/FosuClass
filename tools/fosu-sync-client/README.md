...
本同步客户端支持在同步时自动识别或手动指定当前需要同步的学期，以防止教务系统未来排课或垃圾测试学期影响抓取结果。

#### 1) 学期选择配置
- 默认情况下，本工具将根据当前运行时间自动推断当前学期：
  - 1月至8月：优先推断为当前学年的第二学期（例如：2026年5月运行，自动推断为 `2025-2026-2`）。
  - 9月至12月：优先推断为当前学年的第一学期。
- 如果日志中出现 `2028-2029-1` 这种异常的未来远期学期，说明教务处的默认首选项有误。请在您的本地 `.env` 文件中配置明确的 `PREFERRED_SEMESTER` 环境变量：
  ```env
