...
def build_online_pdf(output: Path) -> None:
    ...
    prompts = [
        "案例 1｜学生  查看2025级计算机类01班第1周课表。→ 只看周三。→ 继续看周三下午，哪里有空教室？",
...
def write_text_files(video_source: Path) -> None:
    ...
    online.write_text(
        ...
        "案例 1｜学生\n"
        "查看2025级计算机类01班第1周课表。\n→ 只看周三。\n→ 继续看周三下午，哪里有空教室？\n\n"
        "在线实测说明：跨日运行时，短句“下午哪里有空教室？”可能被解释为当天；最终问法只补充“继续看周三下午”，仍沿用同一会话且不重复班级与周次。\n\n"
        "案例 2｜教师多人协同\n"
        ...
        "案例 4｜教学管理者\n"
        "未来四周谁的教学负载最高？\n→ 看他的课表。\n→ 再检查他的风险。\n\n"
        "口径：底层校园课表服务已在真实校园场景投入使用，累计服务用户达到 1500+；本次参赛作品在这些真实需求基础上进一步扩展为校园教学时空协同智能体。\n",
        encoding="utf-8",
        newline="\n",
    )
