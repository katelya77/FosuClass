/**
 * CampusTools 交付数据层：加载 competition-demo-v3 匿名演示数据并建立索引。
 *
 * 安全约束（比赛铁律）：
 * - 默认且仅允许读取 competition-demo-v3 匿名数据集；
 * - 禁止任何“比赛接口失败后回退读取 production 真实数据”的行为；
 * - 数据文件名校验：basename 必须为 competition-demo-v3.json，否则拒绝加载；
 * - 不存在面向历史演示数据或真实生产数据的静默回退。
 */

const fs = require("fs");
const path = require("path");

const DEFAULT_DATA_PATH = path.join(__dirname, "..", "..", "data", "competition-demo-v3.json");

let cached = null;

function assertCompetitionData(filePath) {
  const base = path.basename(filePath);
  if (base !== "competition-demo-v3.json") {
    const err = new Error(`拒绝加载非当前匿名数据文件: ${base}（仅允许 competition-demo-v3.json）`);
    err.code = "DATA_GUARD";
    throw err;
  }
}
