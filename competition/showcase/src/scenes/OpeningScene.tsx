...
const BEATS = { nodes: 0.7, connections: 2.4, focus: 5.4, brand: 7.1 };

/** 叙事字幕：无旁白也能看懂的画面自解释文案（Phase 2.5 三步显影） */
const NARRATION = [
  "一所校园的学期，始于无数分散的教学时空。",
  "系统逐一建立课程与教师、班级、教室、时间的关系。",
  "原本分散的信息，被连接并理解。",
];
 ...
 * 0~7.1s：六类实体 → 四类课程关系逐一建立 → 教室落到校区 → 品牌显影；
 * 7.1~10s：品牌保持 + 背景关系降至 12% + 环境呼吸。
  ...
  return (
    <div className="stage-safe flex">
      <SceneCamera shot={{ scale: camScale }} className="relative h-full w-full">
        {/* 教学时空场：六个实体、五条关系；brand 时降低到 12% */}
        <div className="absolute inset-0">
          <CampusTemporalGraph beats={BEATS} local={local} dimAtBrand={brandShown} />
        </div>
