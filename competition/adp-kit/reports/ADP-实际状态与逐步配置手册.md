...
- 2026-08-09 本轮再次实时确认：Chrome 登录有效，ADP 首页可打开；应用列表只有一个标准模式应用，名称仍为“校园智序-小序”，状态为“未上线”；同一空间还打开了“01-多维课表查询”编辑页。
- 浏览器扩展能够准确枚举页面并从应用列表进入“应用设置”，但复杂配置页的 DOM 状态读取连续超时。网页端 ChatGPT 应先重新识别当前页面文字和 DOM；若控制链路仍不稳定，按本手册逐检查点指导用户操作，不得机械重复旧坐标，也不得把未保存的配置写成已完成。
...
| 工作流 | 有 01、02、03，均待发布；没有 04 | 依序完善 01，再完善 02/03，新建 04 |
| 工作流 01 | 只有开始和结束；开始节点仅有 entity_type、entity_name、date_text、week | 补 weekday、period_scope、campus，搭建完整链路 |
| Widget | 空间内没有“校园任务结果卡” | 新建通用 Widget，至少接入 01、02 |
| 评测 | 评测集 0、评测任务 0 | 导入 80 条评测，创建任务并复测 |
| 发布 | 未上线，未做最终发布 | 只允许调试/测试版本；最终发布等待用户总确认 |

需要特别纠正的差异只有两项基础配置：应用名中的连接符，以及最大输出长度 4000。其余基础配置不要无意义反复保存。
...
当前 01 只有开始/结束，开始节点仅四个输入。先在开始节点补齐完整输入：
