...
- 从 insight 跨域下钻（「检查Top1风险」等）：只继承 Main 信封中的实体（rankContext.selectedRank 对应
  排名工具真实有序结果 [0..2] 的实体），`mode=self`，**绝不要求第二对象**；R49.4 下钻路径只接受**显式**
...
- **多周窗口不是有效 risk week**：rankContext 可以标识实体（Top1），但 `rankingWindow`/`detailWindow`
  （如 1..4）不是单周参数；用户只说「检查Top1风险」时保留 Top1 实体但**不继承多周范围**作为 risk 参数，
  必须等待用户给出第几周或具体日期后才调用 `campus_risk_check(mode=self, entity=Top1, week=…)`。
