...
function sections(payload) {
  return payload.sections.map((section, index) => `<section class="module module-${esc(section.kind || (payload.variant === "overview" && index === 0 ? "metric" : "legacy"))}">
...
for (const name of fs.readdirSync(INPUT).filter((name) => name.endsWith(".json"))) {
  ...
  payloads.push([name.replace(/\.json$/, ".html"), payload]);
}
const riskWrap = structuredClone(payloads.find(([name]) => name === "risk.html")[1]);
riskWrap.title = "教师025（负载Top1）未来四周跨校区赶场风险";
riskWrap.summary = "0 冲突 · 4 转场预警 · 最短转场窗口 20 分钟";
riskWrap.context = "第 1–4 周 · 当前示例：校区D D1-106 → 校区A A1-101";
riskWrap.sections = [
  { title: "已核验结果", kind: "metric", rows: [
    { label: "课程冲突", value: "0" },
    { label: "转场预警", value: "4" },
    { label: "最短转场窗口", value: "20 分钟" },
  ] },
  { title: "空间转场示例", kind: "route", rows: [
    { label: "校区D → 校区A", value: "D1-106 → A1-101", badge: "20 分钟" },
  ] },
];
riskWrap.actions = [];
const collaborationWrap = structuredClone(payloads.find(([name]) => name === "collaboration.html")[1]);
collaborationWrap.title = "教师005 / 006 / 014 · 第1周周四上午共同空闲";
collaborationWrap.summary = "3 位教师 · 周四上午 · 63 → 7 → A1-201";
collaborationWrap.context = "第 1 周 · 周四第 1–4 节 · 容量不少于 120 座";
collaborationWrap.sections = [
  { title: "推荐方案", kind: "recommendation", rows: [
    { label: "周四上午", value: "第 1–4 节", badge: "唯一共同空闲" },
  ] },
  { title: "空间收敛", kind: "recommendation", rows: [
    { label: "初始可用", value: "63 间" },
    { label: "满足 ≥120 座", value: "7 间" },
    { label: "首选教室", value: "A1-201 · 120 座", badge: "推荐" },
  ] },
];
collaborationWrap.actions = [];
payloads.push(["verified-risk-title-wrap.html", riskWrap], ["verified-collaboration-title-wrap.html", collaborationWrap]);
for (const [name, payload] of payloads) fs.writeFileSync(path.join(OUTPUT, name), page(payload), "utf8");
process.stdout.write(`Final Widget visual evidence HTML generated: ${payloads.length}\n`);
