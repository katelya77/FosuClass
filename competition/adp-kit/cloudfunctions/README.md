...
本目录是 `mcp/campus-tools-mcp/` 的 CloudBase 托管运行时包装，用于独立的匿名比赛测试环境。`src/` 与 `mock-data/competition-demo-v1.json` 是生成副本；权威实现仍位于 MCP 服务和顶层 mock-data 目录，禁止在这里单独修改业务逻辑。
...
CloudBase HTTP Function 固定监听 `9000`。`index.js` 在冷启动时强制 token 模式并校验数据版本：缺少 `CAMPUS_API_TOKEN` 会拒绝启动，数据只接受 `competition-demo-*`；没有 production 数据路径或失败回退。
