...
CloudBase HTTP Function 固定监听 `9000`。`index.js` 在冷启动时强制 token 模式并校验数据版本：缺少 `CAMPUS_API_TOKEN` 会拒绝启动；显式设置 `CAMPUS_DATA_PATH` 时尊重该值，否则按 `CAMPUS_DEMO_DATA_VERSION`（默认 `competition-demo-v3`，可 `competition-demo-v2`）读取部署包内对应匿名数据集；数据只接受 `competition-demo-*`；没有 production 数据路径或失败回退。部署包内保留 v2/v1 文件仅供显式回滚，运行时默认不会回退旧版本。
...
- 数据版本：`competition-demo-v3`（R50.0 V3 runtime cutover；部署包内同时保留 v2/v1 仅供显式回滚）
