...
test("get_academic_context: 学期首日 = 第1周周一", () => {
  const env = callTool("get_academic_context", { date: "2026-03-02" });
  assertEnvelope(env);
  assert.equal(env.success, true);
...
test("query_schedule: 日期模式（2026-03-02 = 第1周周一）", () => {
  const env = callTool("query_schedule", {
    entityType: "class",
    entityName: "2025级A班",
    date: "2026-03-02",
...
test("generate_day_plan: 演示用户001 周五（含课程与建议）", () => {
  const env = callTool("generate_day_plan", { visitorId: "visitor-demo-001", date: "2026-03-06" });
  assertEnvelope(env);
  assert.equal(env.success, true);
...
test("generate_day_plan: 用户不存在", () => {
  const env = callTool("generate_day_plan", { visitorId: "nobody", date: "2026-03-06" });
  assert.equal(env.success, false);
  assert.equal(env.error.code, "ENTITY_NOT_FOUND");
});
  ...
  const callRes = await (
    await fetch(`${base}/mcp`, {
      ...
      body: JSON.stringify({
        ...
        id: 2,
        method: "tools/call",
        params: { name: "get_academic_context", arguments: { date: "2026-03-02" } },
      }),
    })
  ).json();
