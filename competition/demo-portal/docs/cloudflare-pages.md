...
中继只连接固定公开上游，不接受任意目标；拒绝 ADP 管理、Swagger/OpenAPI、admin 等路径；不转发 Cookie、Authorization 或 Set-Cookie；除小型 `webim/config.js` 的公开域名重写外，响应体保持流式传输。中继不可用时页面保留案例与上下文，并自动显示官方外开入口，不出现空白 iframe。
