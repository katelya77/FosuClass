...
- 默认不得直接部署生产环境。仅当用户在当前任务中明确要求全自动部署或明确授权生产发布时，Agent 才可执行生产部署；该授权只适用于用户点名的目标环境与本次已确认的变更范围，不得扩展到其他服务、数据或环境。
- 获得上述生产授权后，必须使用仓库现有的受控发布工作流或发布工具，并在执行前锁定目标环境、Git 提交与发布版本，运行适用的必跑验证，确认备份/上一版回滚点可用；不得绕过鉴权、审计、幂等、人工确认参数或安全门禁。
- 全自动生产发布必须等待任务进入终态，并执行健康检查、版本/提交指纹核对及关键业务只读探针。合并、推送、触发工作流或命令返回不等于发布成功；验收失败时应停止后续写入并按既有机制回滚，随后如实报告证据。
